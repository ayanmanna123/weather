...
export default function WeatherApp() {
  ...
  const getWeatherIcon = (condition, size = 24) => {
    ...
    if (conditionLower.includes("rain")) {
      ...
      return (
        <CloudSnow
          size={size}
          className={darkMode ? "text-blue-200" : "text-blue-400"}
        />
      );
    } else if (
  ...
  return (
    <div
      className={`min-h-screen ${getBackgroundClass()} transition-colors duration-500`}
    >
      <div className="container mx-auto px-4 py-8">
        ...
        <header className="mb-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <h1
              className={`text-2xl font-bold mb-4 md:mb-0 ${darkMode ? "text-white" : "text-gray-800"}`}
            >
              Weather App
            </h1>
        ...
        {weather && !loading && (
          <div className="space-y-6">
            {/* Current weather */}
            <div
              className={`p-6 rounded-xl shadow-lg ${darkMode ? "bg-gray-800 text-white" : "bg-white text-gray-800"}`}
            >
              <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
        ...
        {/* Footer */}
        <footer
          className={`mt-12 text-center text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}
        >
...
function generateMockWeatherData(city = "London") {
  ...
  const current = {
    ...
    condition: ["Sunny", "Partly Cloudy", "Cloudy", "Light Rain", "Clear"][
